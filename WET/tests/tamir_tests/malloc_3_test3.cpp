...
// replace blocks_list_head with your blocks_list_head of linked list of SBRK MetaData
// replace blocks_list_head_MMAP with your blocks_list_head of linked list of MMAP MetaData
#include <assert.h>
#include <iostream>
#include "malloc_3.cpp"

//raplce main.cpp with your malloc_3.test
MallocMetaData * tail2 (){
    MallocMetaData *meta = blocks_list_head;
    if (!meta) return meta;
    while (meta->next){
        meta = meta->next;
    }
...
int main() {
    ...
    smalloc(0);
    smalloc(100000001);
    std::cout<<_num_allocated_bytes()<<std::endl;
    ...
    assert(tail2() == (MallocMetaData*)p4 - 1);
    assert(blocks_list_head == (MallocMetaData*)p1 - 1);
    ...
    assert(tail2() == (MallocMetaData*)p1 - 1);
    assert(blocks_list_head == (MallocMetaData*)p1 - 1);
    ...
    assert(_num_allocated_bytes() == 7500);
    assert(_num_meta_data_bytes() == 6*META_SIZE);
    assert(tail2() == (MallocMetaData*)p6 - 1);
    assert(blocks_list_head == (MallocMetaData*)p1 - 1);
    p5 = smalloc(1000);
    ...
    assert(_num_free_bytes() == 1000);
    assert(_num_allocated_blocks() == 4);
    assert(_num_allocated_bytes() == 7500 + 2*META_SIZE);
    assert(_num_meta_data_bytes() == 4*META_SIZE);
    ...
    assert(_num_allocated_bytes() == 9500);
    assert(_num_meta_data_bytes() == 7*META_SIZE);
    assert(tail2() == (MallocMetaData*)p7 - 1);
    assert(blocks_list_head == (MallocMetaData*)p1 - 1);
