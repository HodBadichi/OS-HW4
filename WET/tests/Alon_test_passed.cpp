...
void malloc3_test_01() {
    ...
    assert(_num_free_bytes() == sizeof(int) * (100));
    int *eleven = (int *) smalloc(sizeof(int) * 11);
...
void malloc3_test_02() {
    ...
    // order: sfree(280+6*data), 80, sfree(190+data)
    assert(_num_free_blocks() == 2);
    ...
    void* huge1 = (void *) smalloc(128 * 1024);
    assert(_num_free_blocks() == 1);
    assert(_num_free_bytes() ==
           sizeof(long long) * 550 + 9 * _size_meta_data());
    assert(_num_allocated_blocks() == 2);
    assert(_num_allocated_bytes() ==
           sizeof(long long) * 550 + 9 * _size_meta_data() + (128 * 1024));
    assert(_num_meta_data_bytes() == _size_meta_data() * 2);

    void* huge2 = (void *) smalloc(128 * 1024 * 2);
    assert(_num_free_blocks() == 1);
    assert(_num_free_bytes() ==
           sizeof(long long) * 550 + 9 * _size_meta_data());
    assert(_num_allocated_blocks() == 3);
    assert(_num_allocated_bytes() ==
           sizeof(long long) * 550 + 9 * _size_meta_data() + (128 * 1024 * 3));
    assert(_num_meta_data_bytes() == _size_meta_data() * 3);
...
void malloc4_test_03() {
    ...
    // fits just right (problem 1 test for exactly 128 sfree bytes is okay for split)
    ...
    sfree(mid); // sfree place should unite problem 2 and become 1000 again
    assert(_num_free_blocks() == 1);
    assert(_num_free_bytes() == 1000);
    assert(_num_allocated_blocks() == 2);
    assert(_num_allocated_bytes() == 1032);
    ...
    // doesnt fit (problem 1 test for exactly 124 sfree bytes shouldn't split)
    ...
    assert(_num_free_bytes() == 0);
    assert(_num_allocated_blocks() == 2);
    assert(_num_allocated_bytes() == 1040);
    assert(_num_meta_data_bytes() == _size_meta_data() * 2);
}
